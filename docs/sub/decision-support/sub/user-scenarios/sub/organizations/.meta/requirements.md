...
- Present `organizations/` for organization-scale routes where centralized data or knowledge, high-volume processing, shared platform ownership, cross-system automation, concurrency, compliance, access policy, auditability, or high-security requirements materially govern the decision.
...
- Keep canonical ownership of data platforms, RAG architecture, CRM/contact-center systems, security tooling, manufacturing systems, workflow engines, and deployment infrastructure outside this user-scenario subtree. Link those owners only where they materially constrain the route.
...
- Every materialized child contains organization-scale constraints that materially change the route beyond a team-level workflow.
- Organization pages do not claim canonical ownership of complete enterprise solution architecture outside decision support.
