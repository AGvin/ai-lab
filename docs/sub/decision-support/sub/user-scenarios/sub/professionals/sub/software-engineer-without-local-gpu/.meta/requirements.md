...
- Keep coding-task model rankings in `catalog/models/selection/decision-guides/software-development/` and agent safeguards in `catalog/models/selection/decision-guides/agents-and-automation/`; this scenario owns the no-local-GPU route trade-off, not generic coding-model selection.
