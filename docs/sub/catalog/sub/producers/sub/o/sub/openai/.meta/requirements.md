...
- Keep model-domain navigation while representing the current physical `produces` edges to GPT, Whisper, OpenAI Skills, ChatGPT, OpenAI Codex, and OpenAI Agents SDK.
...
- Link GPT, Whisper, OpenAI Skills, ChatGPT, OpenAI Codex, and OpenAI Agents SDK from the producer's physically materialized `produces` relations; do not link Skill Creator as a standalone catalog node.
- Include the official website and GitHub organization.

## Validation

- The OpenAI/GPT, OpenAI/Whisper, OpenAI/OpenAI Skills, OpenAI/ChatGPT, OpenAI/OpenAI Codex, and OpenAI/OpenAI Agents SDK `produces` / `produced-by` relation pairs are physically present at both endpoints and semantically consistent.
