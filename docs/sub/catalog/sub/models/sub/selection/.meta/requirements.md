...
- Do not collapse recommendation, evidence, deployment context, classification, price, or benchmark results into one unsupported aggregate score.
...
- Recommendation labels are scoped to an explicit task, constraints, and evidence state.
- No broad infrastructure lifecycle or solution-architecture guide is migrated into model selection merely because models are components.
