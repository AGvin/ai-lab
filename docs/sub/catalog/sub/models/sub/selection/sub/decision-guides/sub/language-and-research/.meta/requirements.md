...
- Include Kimi K3 as a capability-first evaluation candidate for reasoning, long-context research/synthesis, large-document knowledge work, and research workflows that can materially benefit from native vision plus a provider-documented 1M-token context window.
- Treat Moonshot AI's Kimi K3 knowledge-work case studies, internal evaluations, benchmark results, and long-context positioning as eligibility evidence rather than independent AI Lab proof of research correctness, grounding, citation quality, or accepted-result superiority.
- Keep compact local/private hypotheses, economical hosted routes, and capability-first hosted/self-hosted routes distinguishable by their intended experiment and evidence boundary; Kimi K3's open weights do not by themselves establish a practical self-hosted route because total/active parameter counts are not memory-fit evidence.
...
- Kimi K3 provider research/knowledge-work claims are not treated as independent grounding, factuality, or citation-quality evidence.
- Kimi K3 open-weight status or 104B active-parameter figure is not treated as sufficient self-hosting evidence.
