...
- Preserve useful concrete model-role and escalation hypotheses from legacy environment-profile pages only after current first-party identity/capability verification; strip GPU class, residency schedule, cloud resource, provider lifecycle, and hardware-topology assumptions unless they are frozen evidence conditions needed for the model comparison.
...
- Recheck mutable hosted availability, aliases, prices, limits, and provider features at decision time rather than embedding them in a durable portfolio rank.
...
- Useful model-role hypotheses are not discarded merely because they were embedded in a hardware/environment profile.
- Legacy environment-profile labels are not reproduced as model-team recommendations without removing deployment/hardware ownership and revalidating model identities/capabilities.
- Every concrete portfolio hypothesis identifies a role relationship and missing workflow evidence rather than asserting a current universal winner.
