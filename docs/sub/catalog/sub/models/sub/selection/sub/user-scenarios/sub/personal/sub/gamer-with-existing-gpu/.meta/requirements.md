...
- Do not recommend purchasing another GPU until the existing card has been measured on intended model artifacts, contexts, modalities, and workloads and a real capability/resource gap is demonstrated.
- Keep GPU-buying advice, hardware capacity-class design, runtime selection, and server architecture outside this scenario's ownership.
- Include a **Hardware-specific model selection** continuation that links the complete `../../../hardware/` journey and `../../../hardware/sub/computers/`; when the reader knows the accelerator ecosystem, route onward to the applicable `computers/nvidia/`, `computers/amd/`, `computers/intel/`, or other selected specialization instead of duplicating its fit matrix here.
- Escalate when measured quality, context, modality support, latency, or memory headroom is the actual limiting factor and hosted or different local routes have better accepted-result economics.
- Link named models and runtime/software products to their canonical catalog owners instead of duplicating profiles.

## Validation

- The scenario starts from owned hardware and does not turn into a GPU purchase guide.
...
- The hardware-specific block links the sibling hardware journey rather than recreating hardware-fit guidance.
- A local desktop service is not presented as operationally equivalent to a managed hosted service without maintenance/availability costs.
