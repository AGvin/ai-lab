...
- Identify Gemma 4 E2B Instruct as a concrete dense instruction-tuned multimodal model in the Gemma 4 generation.
- Preserve intrinsic model facts and official artifact identity from the legacy E2B documentation.
...
- Use `Gemma 4 E2B Instruct` as the page title.
- Link the canonical Gemma 4 generation.
- Preserve the dense architecture classification and the 2B effective-parameter identity.
...
- Preserve the official `google/gemma-4-e2b-it` model page.
- Preserve the official `google/gemma-4-e2b-it-qat-q4_0-unquantized` QAT distribution and the legacy published size of approximately 2.17 GB.
...
- State that `E2B` denotes effective parameters and that a distinct total-parameter figure should be added only when independently verified.
