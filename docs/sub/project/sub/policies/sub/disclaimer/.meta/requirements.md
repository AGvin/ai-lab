...
- Make the evaluation boundary explicit: results can materially depend on the exact model/version/artifact or hosted snapshot, provider/runtime, hardware, operating system, drivers, and deployment configuration, numerical precision or quantization, prompts, system instructions, sampling parameters, context and tool access, datasets and representative inputs, evaluation method, acceptance criteria, and evaluation date.
