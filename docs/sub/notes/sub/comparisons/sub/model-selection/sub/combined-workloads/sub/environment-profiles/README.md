...
- [Qwen3 8B](../../../../../../../../../software/sub/models/sub/alibaba/sub/qwen/sub/qwen3/sub/8b/) — economical local baseline; current profiles evaluate the official `Q4_K_M` GGUF artifact.
- [Qwen3 14B](../../../../../../../../../software/sub/models/sub/alibaba/sub/qwen/sub/qwen3/sub/14b/) — resident local generalist candidate; current profiles evaluate the official `Q4_K_M` GGUF artifact.
- [Qwen3 30B-A3B](../../../../../../../../../software/sub/models/sub/alibaba/sub/qwen/sub/qwen3/sub/30b-a3b/) — higher-capacity local candidate; current profiles evaluate the official `Q4_K_M` GGUF artifact.

Published weight or file size is not peak VRAM. Runtime buffers, KV cache, context, batching, offload, drivers, and concurrent services require separate measurement. See [Local Model Selection by VRAM](../../../local-models-by-vram/).
...
| --- | --- | --- | --- |
| One 24 GB GPU | Qwen3 14B Q4_K_M | Qwen3 30B-A3B sequentially or hosted model | VRAM headroom and switching |
| Two 24 GB GPUs | Qwen3 30B-A3B on one GPU | Second GPU specialist or hosted route | Concurrency versus sharding |
| CPU-only | Qwen3 8B Q4_K_M | Luna or DeepSeek V4 Flash | Latency and memory bandwidth |
| Cloud-only | Luna or DeepSeek V4 Flash | Terra, Gemini, Sol, Sonnet, or specialist | Provider cost and data boundary |
| Hybrid local and hosted | Qwen3 14B Q4_K_M | DeepSeek, Terra, Gemini, Sol, Sonnet | Privacy with selective escalation |
| Always-on local | Qwen3 14B Q4_K_M | Remote task specialist | Predictable availability and idle cost |
...
| Low-budget | Qwen3 8B or 14B | DeepSeek, then one bounded stronger route | Cost per accepted result |
| Low-latency | Preloaded Qwen3 14B or Luna | Pre-warmed Terra or multimodal Flash | Tail latency and cold starts |
...
Private text work, coding, orchestration, occasional speech processing, and intermittent media tasks on RTX 3090- or RTX 4090-class hardware.

### Candidate assignment

- **Resident generalist:** Qwen3 14B Q4_K_M.
- **Higher-capacity local text route:** Qwen3 30B-A3B Q4_K_M, loaded only after unloading the resident model.
...
- **Multimodal route:** Gemini 3.6 Flash when the required input modality is not covered locally.
- **Image generation:** hosted endpoint for infrequent work, or a separately measured local worker after unloading the language model.
...
- Do not keep speech or image services resident merely because one test request succeeded.

### Quality and fallback

Use the local route only for quality tiers it passes. Escalate repeated omission, tool, factuality, or completion failures to a different model family or human review rather than repeating the same route indefinitely.

## Two 24 GB GPUs

### Intended workload

Concurrent local text work plus speech, perception, evaluation, or generation without unloading the primary service for every specialist call.

### Candidate assignment

- **GPU 0:** Qwen3 30B-A3B Q4_K_M as the text core after exact validation.
- **GPU 1:** one measured specialist at a time, such as Whisper, Community-1, FLUX.1-schnell, or another exact perception model.
...
Measure per-GPU VRAM, host RAM, PCIe or interconnect transfer, context cache, concurrency, queueing, and cross-service contention. Preserve a hosted fallback because two local GPUs remain one host, power, storage, and software failure domain.
...
- **Local baseline:** Qwen3 8B Q4_K_M.
- **Optional larger batch route:** Qwen3 14B Q4_K_M only after latency and RAM validation.
...
- **Local privacy and routing core:** Qwen3 14B Q4_K_M.
...
- **Always-on core:** Qwen3 14B Q4_K_M as generalist, router, summarizer, and low-risk worker only for roles it passes.
...
Keep one local service rather than separate planner, router, reviewer, and worker models when the same artifact passes each role. Split roles when review must be independent, permissions differ, concurrency suffers, or a specialist materially improves cost per accepted result.

Monitor loaded artifact, context isolation, queue depth, disk, memory, restart behavior, remote health, and timeouts. Expose a clear degraded state instead of pretending remote capability remains available.
...
- **Local baseline:** Qwen3 8B on CPU or Qwen3 14B on an available GPU.
...
- **Local zero-network route:** preloaded Qwen3 14B with bounded context and concurrency.
...
Measure queue delay, upload, time to first token or audio, tool latency, terminal artifact latency, p50, p95, p99, timeout, cancellation, and retry. Avoid model swapping, on-demand GPU startup, long deliberation, councils, and synchronous media generation in the critical path.
