...
- Explain that the current materialized subset includes `model-loading/`, `model-formats-and-conversion/`, `cpu-inference/`, `gpu-inference/`, and `offloading/`; `accelerator-inference/` remains a selected logical child until source-backed content/configuration/navigation value justifies materialization.
