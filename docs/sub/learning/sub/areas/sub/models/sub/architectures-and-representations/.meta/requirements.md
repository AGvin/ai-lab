...
- Explain that the current materialized subset focuses on `transformers-and-attention/` because legacy attention/Transformer/encoder-decoder sources contain source-backed pedagogical and practical architecture guidance ready for migration.
...
- Keep concrete framework APIs, model/version facts, hardware/runtime compatibility, benchmark measurements, and recommendation outcomes with their applicable catalog/evidence/decision owners.

## Validation

- Learning content remains aligned with canonical architecture concepts without duplicating their ownership.
- Architecture is not conflated with task, training objective, runtime implementation, or model quality.
- Current navigation exposes only materialized selected children.
- Mutable compatibility/performance claims remain outside timeless learning truth.
