...
- Materialize only source-backed selected descendants needed by current packages: `fine-tuning/`, `supervised-post-training/`, and `preference-and-alignment/`.
...
- Evaluate target improvements together with important retained capabilities, representative failure cases, and regressions; training loss or a single proxy score alone is not sufficient evidence.
