...
- Treat `plugins/` as the reusable owner for the stable AI-host extension concept shared across package-, bundle-, and module-style plugin ecosystems. Do not define all plugins by one manifest, directory layout, component set, marketplace, or installation mechanism.
- Treat Agent Plugins as one current portable plugin standard rather than the universal definition of `plugin`. Its normative `plugin.json` schema, portable component set, extension namespaces, conformance rules, versioning, and client requirements remain specification-owned; vendor-specific plugin formats can coexist with it.
...
- Explain discovery/registration as how a host learns which extensions/capabilities are available. Discovery may use filesystem scanning, registries, manifests, marketplaces, configuration, bundled assets, package managers, modules, or platform APIs; no one mechanism is universal.
...
- Treat third-party extension content as supply-chain input. Markdown instructions, prompts, manifests, scripts, hooks, binaries, templates, MCP configuration, modules, and remote references can all influence behavior or data flow.
...
- Keep mutable host support matrices, install paths, marketplace status, command syntax, current preview/GA state, plugin manifests, supported metadata fields, component sets, extension APIs, and compatibility bugs with concrete platform/catalog/evidence owners.
...
- Keep `agent-skills/` and `plugins/` as the currently selected direct children. Do not infer vendor-specific plugin/package leaves solely from product terminology; each additional concept path requires a distinct reusable semantic need and explicit architecture selection.
...
- Use canonical entity references as research inputs for portable Agent Skills, Agent Plugins, and host-specific plugin packaging while preserving their distinct concept/specification/catalog boundaries.
...
- Portable concepts are not described using one vendor's install path, manifest, command syntax, permission model, marketplace, component set, or module API as universal behavior.
- Agent Plugins is not treated as the definition of every AI plugin or as proof that all plugin ecosystems are format-compatible.
