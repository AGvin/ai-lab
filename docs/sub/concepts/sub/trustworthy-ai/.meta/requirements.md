...
- Keep the currently materialized `privacy/`, `alignment-and-control/`, and `information-integrity/` descendants distinct. `security-and-resilience/` is now explicitly selected for materialization by the active prompt-injection package; other broad trustworthy-AI areas should appear in generated navigation only after they gain substantive canonical material.
- Keep the selected `security-and-resilience/prompt-injection/indirect-prompt-injection/` hierarchy explicit rather than inferring a flat list from legacy pages. Prompt injection is a security attack/vulnerability family; indirect prompt injection is its selected specialization for external/resource-controlled instruction paths.
- Do not infer or materialize still-unselected legacy-named child nodes such as trust boundaries, least privilege, sandboxing, secret handling, retrieval poisoning, guardrails, data residency, content moderation, jailbreaking, or data poisoning until the architecture explicitly selects their exact destinations.
...
- Selected prompt-injection/provenance descendants are no longer incorrectly blocked by stale gap wording, while still-unselected legacy leaves are not inferred/materialized from broad parents.
