...
- Do not infer or materialize still-unselected legacy-named child nodes such as trust boundaries, least privilege, sandboxing, secret handling, guardrails, data residency, or content moderation until the architecture explicitly selects their exact destinations.
...
- Selected prompt-injection, data-poisoning, retrieval-poisoning, jailbreaking, and provenance descendants are no longer incorrectly blocked by stale gap wording, while still-unselected legacy leaves are not inferred/materialized from broad parents.
