...
- Keep the selected `security-and-resilience/prompt-injection/indirect-prompt-injection/` hierarchy and sibling `security-and-resilience/data-poisoning/` explicit rather than inferring a flat list from legacy pages. Prompt injection owns inference/application instruction-manipulation attacks; data poisoning owns adversarial manipulation of persistent learning/adaptation data.
- Do not infer or materialize still-unselected legacy-named child nodes such as trust boundaries, least privilege, sandboxing, secret handling, retrieval poisoning, guardrails, data residency, content moderation, or jailbreaking until the architecture explicitly selects their exact destinations.
...
- Selected prompt-injection, data-poisoning, and provenance descendants are no longer incorrectly blocked by stale gap wording, while still-unselected legacy leaves are not inferred/materialized from broad parents.
