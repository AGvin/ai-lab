...
- Keep `controlnet/` as the currently selected direct child for the specific ControlNet conditioning architecture. Do not classify every structural/spatial conditioning, adapter, guidance, image-to-image, or control mechanism as ControlNet merely because it influences diffusion generation.
...
- Keep concrete diffusion/ControlNet checkpoints, latent autoencoders, conditioning/preprocessing models, schedulers/samplers, guidance/control scales, step ranges, seeds, runtime implementations, benchmark results, and workflow/model-selection recommendations with their applicable catalog, runtime, evidence, learning, or decision owners.
...
- Generic diffusion conditioning remains distinct from the selected specific `controlnet/` architecture; unrelated control/adaptation methods are not mislabeled as ControlNet.
- Concrete diffusion/ControlNet models, preprocessors, samplers, control settings, benchmark outcomes, and deployment/workflow recipes remain outside the abstract architecture owners.
