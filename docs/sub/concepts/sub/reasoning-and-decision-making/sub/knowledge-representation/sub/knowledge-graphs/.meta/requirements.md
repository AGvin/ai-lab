...
- Keep the canonical `ai-engineering/architectures-and-patterns/rag/graph-rag/` concept separate from knowledge-graph representation. GraphRAG may use, construct, traverse, or summarize a knowledge graph or another graph structure as part of retrieval-augmented generation, but a knowledge graph does not imply GraphRAG and GraphRAG does not require every graph substrate to be a formal knowledge graph.
