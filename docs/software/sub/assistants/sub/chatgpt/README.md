...
ChatGPT is OpenAI's conversational AI product for chat, research, files, multimodal work, connected applications, reusable skills, and tool-enabled workflows.
...
ChatGPT supports reusable skills on eligible plans and surfaces. A skill can be created in conversation, edited in the product, uploaded, shared where permitted, or installed through a compatible plugin.
...
OpenAI plugins are installable capability packages that may contain skills, apps, app templates, connectors, or MCP configuration. The exact components and permissions differ by plugin.
...
- connected apps and granted scopes;
- file and network access;
- approval behavior for consequential actions;
- whether the same configuration is available on desktop, web, and mobile;
- date of verification.

Do not assume that a skill grants access to tools or connected systems. The host, workspace policy, user authorization, and active connectors determine which operations are available.
